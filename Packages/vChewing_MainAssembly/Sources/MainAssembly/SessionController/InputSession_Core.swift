...
public final class InputSession: SessionProtocol {
  ...
  private unowned static var _current: (any SessionProtocol)?

  private unowned var theClient: ClientObj?
}
...
extension InputSession {
  ...
  public func activateServer(_ sender: Any!) {
    asyncOnMain { [weak self] in
      ...
      Self.current = self
      let guardedClient = sender as? ClientObj
      if let guardedClient, let senderBundleID: String = guardedClient.bundleIdentifier() {
        vCLog("activateServer(\(senderBundleID))")
        self.theClient = guardedClient
  ...
  public func deactivateServer(_ sender: Any!) {
    asyncOnMain { [weak self] in
      ...
      self.inputController?.superDeactivateServer(sender)
    }
  }
  ...
  public func client() -> (any IMKTextInput & NSObjectProtocol)! {
    inputController?.client() ?? theClient ?? nil
  }
